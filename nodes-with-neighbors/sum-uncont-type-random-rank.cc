// 195 GB/s with alignment, randomw swap [N/100] times
// 3.27 Billion nodes/s
// 100 GB/s with alignment, randomw swap [N/10 ] times
// 1.67 Billion nodes/s
...
void random_swap(int tot)
{
    ...
    for (int i = 0; i < tot; i++) {
        for (int j = 0; j < thread_num; j++) {
            auto id1 = dis(gen);
            auto id2 = dis(gen);
            std::swap(rank[j][id1], rank[j][id2]);
        }
    }
}
