...
#pragma omp parallel for default(shared) reduction(+: ret)
    for (auto i = 0; i < maxn; i++) {
        ...
        // _mm_prefetch((const char*)(a + a[i + 5].next[0]), _MM_HINT_T1);
        // _mm_prefetch((const char*)(a + a[i + 5].next[1]), _MM_HINT_T1);
        // _mm_prefetch((const char*)(a + a[i + 5].next[2]), _MM_HINT_T1);
        // _mm_prefetch((const char*)(a + a[i + 5].next[3]), _MM_HINT_T1);
